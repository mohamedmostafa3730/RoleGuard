...
<body class="bg-slate-50 p-12">
    <div class="max-w-4xl mx-auto">
        ...
        <div
            ...
            @can('create-permissions') {{-- Note: Changed from create-roles to match the page --}}
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-6 py-4 text-right flex justify-end gap-4">
                            <a href="{{ route('admin.permissions.edit', $permission->id) }}"
                                class="text-[10px] font-black text-slate-400 hover:text-blue-600 uppercase">Edit</a>
                            <form action="{{ route('admin.permissions.destroy', $permission->id) }}" method="POST"
                                onsubmit="return confirm('Delete this permission?');">
                                @csrf @method('DELETE')
                                <button
                                    class="text-[10px] font-black text-rose-400 hover:text-rose-600 uppercase">Remove</button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
